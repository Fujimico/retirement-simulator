...
// ── 2バケツシミュレーション
function simulate({
  currentAge, totalAssets, investedAssets, assetBuckets,
  ...
  riskOffEnabled, riskOffAge, riskOffMovePct,
}) {
  const MAX = 100;
  const points = [];

  const bucketMap = Object.fromEntries((assetBuckets ?? []).map(b => [b.key, b]));
  const cashLikeBase = bucketMap.cashLike?.amount ?? Math.max(totalAssets - investedAssets, 0);
  const stableBase = bucketMap.stable?.amount ?? investedAssets;
  const growthBase = bucketMap.growth?.amount ?? 0;

  let cashBucket = cashLikeBase * 1e4;
  let stableBucket = stableBase * 1e4;
  let growthBucket = growthBase * 1e4;

  const stableBaseRate = bucketMap.stable?.rate ?? returnRate;
  const growthBaseRate = bucketMap.growth?.rate ?? returnRate;
  const baseInvested = stableBase + growthBase;
  const baseWeightedReturnRate = baseInvested > 0
    ? ((stableBase * stableBaseRate) + (growthBase * growthBaseRate)) / baseInvested
    : returnRate;
  const returnDelta = returnRate - baseWeightedReturnRate;
  const stableRate = stableBaseRate + returnDelta;
  const growthRate = growthBaseRate + returnDelta;

  let _assetShockApplied = false;
  let _riskOffApplied = false;

  const saleProceeds = saleEnabled
    ? calcAfterTax(saleGross * 1e4, saleBookValue * 1e4, saleTaxType) : 0;

  const getInvestTotal = () => stableBucket + growthBucket;
  const getStableShare = () => {
    const investTotal = getInvestTotal();
    if (investTotal > 0) return stableBucket / investTotal;
    if (baseInvested > 0) return stableBase / baseInvested;
    return 0.5;
  };
  const addToInvestBuckets = (amount) => {
    if (!amount) return;
    const stableShare = getStableShare();
    stableBucket += amount * stableShare;
    growthBucket += amount * (1 - stableShare);
  };
  const withdrawFromInvestBuckets = (amount) => {
    let remaining = Math.max(amount, 0);
    if (!remaining) return;
    const fromStable = Math.min(Math.max(stableBucket, 0), remaining);
    stableBucket -= fromStable;
    remaining -= fromStable;
    growthBucket -= remaining;
  };
  const topUpCashFromInvest = (amount) => {
    let remaining = Math.max(amount, 0);
    if (!remaining) return 0;
    const fromStable = Math.min(Math.max(stableBucket, 0), remaining);
    stableBucket -= fromStable;
    cashBucket += fromStable;
    remaining -= fromStable;
    const fromGrowth = Math.min(Math.max(growthBucket, 0), remaining);
    growthBucket -= fromGrowth;
    cashBucket += fromGrowth;
    remaining -= fromGrowth;
    return fromStable + fromGrowth;
  };
  ...
  for (let age = currentAge; age <= MAX; age++) {
    ...
    const currentAssets = cashBucket + getInvestTotal();
    const investRatio = currentAssets > 0 ? getInvestTotal() / currentAssets : 0;
    ...
    const investedContribution = annualIncome * incomeRatio + windfallIncome * windfallRatio;
    addToInvestBuckets(investedContribution);
    cashBucket += annualIncome * (1 - incomeRatio) + windfallIncome * (1 - windfallRatio);

    if (riskOffEnabled && !_riskOffApplied && riskOffAge != null && age === riskOffAge) {
      const moveAmount = Math.max(growthBucket, 0) * ((riskOffMovePct ?? 0) / 100);
      growthBucket -= moveAmount;
      stableBucket += moveAmount;
      _riskOffApplied = true;
    }
    ...
    if (stressExtraMonthlyFrom != null && age >= stressExtraMonthlyFrom && stressExtraMonthly > 0) {
      ...
      withdrawFromInvestBuckets(extraExp - fromCashExtra);
    }

    stableBucket *= (1 + stableRate / 100);
    growthBucket *= (1 + growthRate / 100);

    const fromCash = Math.min(cashBucket, annualExpense);
    cashBucket -= fromCash;
    withdrawFromInvestBuckets(annualExpense - fromCash);

    const cashFloor = monthlyExp * inflFactor * cashBufferMonths * 1e4;
    if (cashBucket < cashFloor) {
      topUpCashFromInvest(cashFloor - cashBucket);
    }
    ...
    points.push({
      ...
      stable: stableBucket,
      growth: growthBucket,
      invested: investTotal,
      cash: cashBucket,
      assets: investTotal + cashBucket,
      investedClamped: Math.max(investTotal, 0),
...
const AssetBucketEditor = ({ buckets, onChange, totalAssets, investedAssets, weightedReturnRate }) => {
  const updateBucket = (key, patch) => {
    onChange(buckets.map(b => b.key === key ? { ...b, ...patch } : b));
  };
  const cardStyle = {
    background: "#0a1520",
  ...
  return (
    <div>
      {buckets.map((b) => {
        const showRate = b.key !== "cashLike";
        return (
          <div key={b.key} style={cardStyle}>
            <div style={{ display: "grid", gridTemplateColumns: showRate ? "1fr 92px 92px" : "1fr 92px", gap: 8, alignItems: "end" }}>
              <div>
                <div style={{ color: b.color, fontSize: 15, fontWeight: 700 }}>{b.label}</div>
                <div style={{ color: "#445566", fontSize: 13, marginTop: 2 }}>{b.sub}</div>
              </div>
              <label style={{ display: "block" }}>
                <div style={{ ...S, fontSize: 13, marginBottom: 3 }}>金額</div>
                <input
                  type="number"
                  min={0}
                  max={999999}
                  step={100}
                  value={b.amount}
                  onChange={(e) => updateBucket(b.key, { amount: Math.max(0, Number(e.target.value) || 0) })}
                  style={{ width: "100%", background: "#060e18", border: "1px solid #1e3a5f", borderRadius: 6, color: "#c8d8e8", padding: "6px 8px", fontSize: 15, outline: "none", fontFamily: "inherit", boxSizing: "border-box" }}
                />
              </label>
              {showRate && (
                <label style={{ display: "block" }}>
                  <div style={{ ...S, fontSize: 13, marginBottom: 3 }}>利回り</div>
                  <input
                    type="number"
                    min={0}
                    max={15}
                    step={0.1}
                    value={b.rate}
                    onChange={(e) => updateBucket(b.key, { rate: Math.max(0, Number(e.target.value) || 0) })}
                    style={{ width: "100%", background: "#060e18", border: "1px solid #1e3a5f", borderRadius: 6, color: "#c8d8e8", padding: "6px 8px", fontSize: 15, outline: "none", fontFamily: "inherit", boxSizing: "border-box" }}
                  />
                </label>
              )}
      ...
      <div style={{ background: "#0a1520", border: "1px solid #1e3a5f22", borderRadius: 7, padding: "8px 10px", marginTop: 10 }}>
        <InfoRow label="総資産（自動集計）" value={fmtFull(totalAssets * 1e4)} color="#e8f0fe" />
        <InfoRow label="運用資産（安定 + 成長）" value={fmtFull(investedAssets * 1e4)} color="#4a9eff" />
        <InfoRow label="全体の想定利回り" value={`年 ${weightedReturnRate.toFixed(1)}%`} color="#4a9eff" />
      </div>
    </div>
  );
};
...
const TERM_TIPS = {
  ...
  assetClasses: "現金・待機資産 / 安定運用資産 / 成長運用資産の3分類で持ち方を置く設定。\n安定と成長だけを運用バケツとして扱い、利回りは加重平均で自動計算します。",
...
export default function App() {
  ...
  const [assetBuckets, setAssetBuckets] = useState([
    { key: "cashLike", label: "現金・待機資産", sub: "普通預金・短期で使う資金", amount: 2000, rate: 0.0, color: "#4adfb0" },
    { key: "stable", label: "安定運用資産", sub: "債券・定期・低リスク運用", amount: 1000, rate: 1.0, color: "#88b8ff" },
    { key: "growth", label: "成長運用資産", sub: "株・投信・ETF・NISAなど", amount: 2000, rate: 5.5, color: "#4a9eff" },
  ]);
  ...
  const [riskOffEnabled, setRiskOffEnabled] = useState(false);
  const [riskOffAge, setRiskOffAge] = useState(65);
  const [riskOffMovePct, setRiskOffMovePct] = useState(50);
  ...
  const cashLikeAssets = assetBuckets.find(b => b.key === "cashLike")?.amount ?? 0;
  const stableAssets = assetBuckets.find(b => b.key === "stable")?.amount ?? 0;
  const growthAssets = assetBuckets.find(b => b.key === "growth")?.amount ?? 0;
  const totalAssets = cashLikeAssets + stableAssets + growthAssets;
  const investedAssets = stableAssets + growthAssets;
  const returnRate = investedAssets > 0
    ? ((stableAssets * (assetBuckets.find(b => b.key === "stable")?.rate ?? 0)) + (growthAssets * (assetBuckets.find(b => b.key === "growth")?.rate ?? 0))) / investedAssets
    : 0;
  const safeInvested = investedAssets;
  ...
  const getCurrentParams = useCallback(() => ({
    ...
    incomeInvestRatioPct, windfallInvestRatioPct,
    riskOffEnabled, riskOffAge, riskOffMovePct,
  }), [currentAge, totalAssets, investedAssets, assetBuckets, incomePhases, loans, oneTimeEvents,
  ...
  const saveScenario = async (name) => {
    ...
    try { await storage.set('scenarios', JSON.stringify(updated)); } catch {}
    ...
    <div className="print-page outer-pad" style={{ minHeight: "100vh", background: bgTheme.bg, color: bgTheme.color, fontFamily: "'DM Mono','Fira Code','Courier New',monospace", padding: "20px 16px", transition: "background 1s, color 0.5s" }}>
      ...
      <div style={{ marginBottom: 18, position: "relative" }}>
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: 15, color: "#334455", marginTop: 3 }}>2バケツ方式 + 3分類資産入力（現金 / 安定 / 成長）</div>
        </div>
            ...
            <table className="print-table" style={{ width: "100%", fontSize: 15, borderCollapse: "collapse" }}>
              <tbody>
                {[
                  ...
                  ["インフレ率", `${inflationRate}%`],
                  ["運用利回り", `${returnRate.toFixed(1)}%`],
                  ["自動リスクオフ", riskOffEnabled ? `${riskOffAge}歳で成長の${riskOffMovePct}%を安定へ` : "OFF"],
                ].map(([label, value]) => (
                  <tr key={label}>
          ...
          <div style={{ background: "linear-gradient(160deg,#080e18,#0a1220)", border: "1px solid #2a3a50", borderRadius: 12, padding: "14px 15px", marginBottom: 12 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(155px, 1fr))", gap: "8px 12px" }}>
              {[
                ...
                { label: "運用利回り",    value: `年 ${returnRate.toFixed(1)}%`, sub: "安定/成長の加重平均" },
                { label: "自動リスクオフ", value: riskOffEnabled ? `${riskOffAge}歳で成長の${riskOffMovePct}%を安定へ` : "OFF", sub: "一回だけ実行" },
        ...
        <div style={{ background: "linear-gradient(160deg,#0d1a12,#0a1810)", border: "1px solid #2adf9033", borderRadius: 12, padding: "14px 15px", marginBottom: 12 }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 15 }}>
              ...
              <tbody>
                {[
                  { label: "🔴 悲観", prefix: `利回り${(returnRate + TRI_PESSIMISTIC.returnDelta).toFixed(1)}% / インフレ${inflationRate + TRI_PESSIMISTIC.inflDelta}%`, result: triPessimistic, color: "#ff8855" },
                  { label: "⚪ 標準", prefix: `利回り${returnRate.toFixed(1)}% / インフレ${inflationRate}%`, result: withSale, color: "#c8d8e8" },
                  { label: "🟢 楽観", prefix: `利回り${(returnRate + TRI_OPTIMISTIC.returnDelta).toFixed(1)}% / インフレ${inflationRate + TRI_OPTIMISTIC.inflDelta}%`, result: triOptimistic, color: "#55cc88" },
      ...
      <div style={{ ...P, marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 10, flexWrap: "wrap", alignItems: "center" }}>
          {[
            { color: "#4a9eff", label: `運用（利回り ${returnRate.toFixed(1)}%）` },
            { color: "#4adfb0", label: "手元（利回りなし）" },
            { color: "#556677", label: "売却なし 合計", dash: true },
            ...(triMode ? [
              { color: "#ff8855", label: `悲観（利回り${(returnRate + TRI_PESSIMISTIC.returnDelta).toFixed(1)}%/インフレ${inflationRate + TRI_PESSIMISTIC.inflDelta}%）`, dash: true },
              { color: "#55cc88", label: `楽観（利回り${(returnRate + TRI_OPTIMISTIC.returnDelta).toFixed(1)}%/インフレ${inflationRate + TRI_OPTIMISTIC.inflDelta}%）`, dash: true },
            ] : []),
            ...(stressMode && stressResult ? [
      ...
      <div className="no-print" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(270px, 100%), 1fr))", gap: 11 }}>
        ...
        <div style={P}>
          <Sec title={<Tip term="bucket2">資産 & 2バケツ配分</Tip>}>
            ...
            <div style={{ fontSize: 14, color: "#556677", marginBottom: 8 }}>
              <Tip term="assetClasses">3分類で金額と想定利回りを置き、運用資産の利回りは自動計算します</Tip>
            </div>
            <AssetBucketEditor
              buckets={assetBuckets}
              onChange={setAssetBuckets}
              totalAssets={totalAssets}
              investedAssets={safeInvested}
              weightedReturnRate={returnRate}
            />
            <div style={{ marginTop: 10, background: "#0a1520", border: "1px solid #1e3a5f22", borderRadius: 7, padding: "9px 10px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: riskOffEnabled ? 8 : 0 }}>
                <div>
                  <div style={{ color: "#c8d8e8", fontSize: 15, fontWeight: 600 }}>自動リスクオフ</div>
                  <div style={{ color: "#556677", fontSize: 13 }}>指定年齢で成長資産の一部を安定資産へ移します</div>
                </div>
                <Toggle value={riskOffEnabled} onChange={setRiskOffEnabled} color="#88b8ff" />
              </div>
              {riskOffEnabled && (
                <>
                  <SliderInput label="リスクオフ開始年齢" value={riskOffAge} min={50} max={90} step={1} unit="歳" onChange={setRiskOffAge} accent="#88b8ff" />
                  <SliderInput label="成長 → 安定 に移す割合" value={riskOffMovePct} min={0} max={100} step={5} unit="%" onChange={setRiskOffMovePct} accent="#88b8ff" />
                  <div style={{ color: "#88a0b8", fontSize: 13, marginTop: -4 }}>
                    {riskOffAge}歳の年に、成長運用資産の {riskOffMovePct}% を一回だけ安定運用資産へ移す前提です。
                  </div>
                </>
              )}
            </div>
            ...
            <div style={{ background: "#0a1520", border: "1px solid #1e3a5f22", borderRadius: 7, padding: "8px 10px", fontSize: 15 }}>
              <InfoRow label="初期の配分" value={`運用 ${investRatioPct}% / 手元 ${100 - investRatioPct}%`} color="#4a9eff" />
          ...
          <Sec title="生活費（年齢帯別）・運用" color="#ff8899">
            ...
            <div style={{ marginTop: 10, background: "#0a1520", border: "1px solid #1e3a5f22", borderRadius: 7, padding: "7px 9px" }}>
              ...
              <InfoRow label="運用資産 利回り（自動計算）" value={`年 ${returnRate.toFixed(1)}%`} color="#4a9eff" />
